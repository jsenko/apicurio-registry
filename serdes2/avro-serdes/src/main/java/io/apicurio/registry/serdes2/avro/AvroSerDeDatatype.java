...
public class AvroSerDeDatatype<DATA> implements GenericSerDeDatatype<Schema, DATA> {
    ...
    public void configure(GenericConfigReader config) {
        var config2 = castOr(config, AvroSerDeConfigReader.class, () -> new AvroSerDeConfigReader(config));
    ...
    @Override
    public void writeData(Headers headers, ParsedSchema<Schema> schema, DATA data, OutputStream out)
            throws Exception {
        if (headers != null) {
