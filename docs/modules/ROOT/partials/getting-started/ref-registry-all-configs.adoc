...
|Available from
|Description
|`registry.rules.semver-branching.mode`
|`semverbranchingmode`
|`disabled`
|`3.0.0`
|Automatically create or update branches for major ('A.x') and minor ('A.B.x') artifact versions. One of 'disabled', 'coerce', or 'strict'. If not disabled, versions must either conform or can be coerced to Semantic Versioning 2 format (https://semver.org)
|`registry.rules.semver-validator.enabled`
|`boolean`
|`false`
|`3.0.0`
