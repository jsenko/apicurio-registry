...
| **Kubernetes cluster** | | |
| `kubernetesopsit`         | Kubernetes operator integration tests            | Kubernetes cluster      |
...
```bash
./integration-tests/run-tests.sh --testProfile smoke --registryHost localhost --registryPort 8080
```

If `--testProfile` is omitted, the default groups are used. See `--help` for all options.
